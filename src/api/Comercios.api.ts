...
const DEFAULT_BANNER = "https://placehold.co/600x200/e8f5e9/1b1b1b?text=Comercio";
...
const extractItems = (payload: unknown): ComercioResponseItem[] => {
  ...
  const listCandidates = ["items", "data", "resultados", "comercios", "contenido", "content"];
...
const mapComercio = (item: ComercioResponseItem, index: number): ComercioCard => {
  ...
  return {
    ...
    logo: toText(getFirst(item, ["logo", "logoUrl", "imagenLogo", "LogoUrl"]), DEFAULT_LOGO),
    rating: toNumber(getFirst(item, ["rating", "calificacion", "Calificacion"])),
    tiempo: toText(getFirst(item, ["tiempo", "tiempoEntrega", "TiempoEntrega"]), "Sin tiempo"),
    envio: toText(getFirst(item, ["envio", "costoEnvio", "CostoEnvio"]), "Sin envio"),
  };
};

const getComercios = async (params: Record<string, string | number>): Promise<ComercioCard[]> => {
