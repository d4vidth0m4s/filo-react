...
const DETALLE_FAKE: DetallePedido = {
  ...
  mensajeEstado:
    'Carlos tiene tu pedido y está a 1.2 km. Llegada estimada: 1:15 PM.',
  pasos: [
...
const AUTH_BRIDGE_URL = import.meta.env.VITE_AUTH_BRIDGE_URL?.trim() ?? '';
...
const UserPerfil: React.FC = () => {
  ...
  async function codeAccessHandler() {
    ...
    setTimeout(() => {
      win?.postMessage({ code: data.codigo }, AUTH_BRIDGE_ORIGIN);
    }, 500);
      ...
      <div className="perfil-layout">
        ...
        <aside className="ticket">
          <div className="ticket-inner">
            ...
            <hr />
            <div className="center">
              <pre className="ascii-logo">{filoAscii}</pre>
            </div>
          </div>
        </aside>
          ...
          {vistaActiva === 'perfil' && (
            <>
              ...
              <h3 className="nombre-usuario">
                {usuario?.nombre} {usuario?.familyName}
              </h3>
